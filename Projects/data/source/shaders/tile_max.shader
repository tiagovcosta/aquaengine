	...
	ps_func =
	{
		...
		"""
			float2 ps_main(PS_INPUT input) : SV_TARGET0
			{
				...
				for(uint i = 0; i < max_blur_radius; i++)
				{
					#if !VERTICAL
						float2 velocity = velocity_texture.Load(uint3((uint)input.position.x * max_blur_radius + i, input.position.y, 0)).xy;
					#else
						float2 velocity = velocity_texture.Load(uint3(input.position.x, (uint)input.position.y * max_blur_radius + i, 0)).xy;
					#endif
